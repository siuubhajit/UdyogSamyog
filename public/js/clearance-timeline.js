...
function renderClearanceTimeline(application, options = {}) {
  ...
  try {
    stageStatuses =
      typeof application.stage_statuses === "string"
  ...
  const currentStage =
    application.current_stage || application.currentStage || "mpcb";
  ...
  const isFinalApproved =
    ...
    safeGetStageDecision("msins") === "Approved";
  ...
  const approvedParallelCount = parallelDepts.filter(
    (d) => safeGetStageDecision(d) === "Approved",
  ).length;
  const isP2Done = approvedParallelCount === 3 || isFinalApproved;
  const isP2Active = ["parallel_scrutiny", "midc", "dish", "fire"].includes(
    currentStage,
  );
  const p2StatusText = isP2Done
    ? "All 3 Approved"
    : isP2Active
      ? `${approvedParallelCount} of 3 approved`
      : isP1Done
        ? "Under Review"
        : "Waiting";
  ...
  const p3StatusText = isP3Done
    ? "Sanctioned"
    : currentStage === "msins"
      ? "In Review"
      : "Waiting";
  const p3Class = isP3Done
    ? "done"
    : currentStage === "msins"
      ? "active"
      : "pending";
  ...
  if (mode === "compact") {
    html = `
      <div class="clearance-timeline clearance-timeline--compact" style="display:flex;align-items:center;gap:12px;background:var(--color-surface-page, var(--surface-card-alt));border:1px solid var(--color-border, var(--line));border-radius:10px;padding:10px 14px;margin-top:6px;">
        <div class="timeline-step timeline-step--${p1Class}" style="display:flex;align-items:center;gap:8px;flex:1;">
          ...
          <div>
            <div style="font-size:0.75rem;font-weight:700;color:var(--color-text-primary, var(--ink));">Phase 1: Environmental</div>
            <div style="font-size:0.7rem;color:${isP1Done ? "var(--ok-ink)" : "var(--ink-secondary)"};font-weight:600;">${p1StatusText}</div>
          </div>
        </div>

        <div style="color:var(--color-border, var(--line-strong));">&rarr;</div>
        ...
        <div class="timeline-step timeline-step--${p2Class}" style="display:flex;align-items:center;gap:8px;flex:1;">
          ...
          <div>
            <div style="font-size:0.75rem;font-weight:700;color:var(--color-text-primary, var(--ink));">Phase 2: Civil, Safety &amp; Fire</div>
            <div style="font-size:0.7rem;color:${isP2Done ? "var(--ok-ink)" : "var(--ink-secondary)"};font-weight:600;">${p2StatusText}</div>
          </div>
        </div>
        ...
        <div class="timeline-step timeline-step--${p3Class}" style="display:flex;align-items:center;gap:8px;flex:1;">
          ...
          <div>
            <div style="font-size:0.75rem;font-weight:700;color:var(--color-text-primary, var(--ink));">Phase 3: Final Approval</div>
            <div style="font-size:0.7rem;color:${isP3Done ? "var(--ok-ink)" : "var(--ink-secondary)"};font-weight:600;">${p3StatusText}</div>
          </div>
        </div>
      </div>
    `;
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:10px;">
              ${parallelDepts
                .map((d) => {
                  const dInfo = {
                    midc: {
                      title: "MIDC Civil & Planning",
                      act: "Building Bylaws & Setbacks",
                    },
                    dish: {
                      title: "DISH Occupational Safety",
                      act: "Factories Act (Sec 16)",
                    },
                    fire: {
                      title: "Maharashtra Fire Services",
                      act: "Life Safety & Hydrant Ring",
                    },
                  }[d];
  ...
  /* ─── 3. OFFICER MODE (Verification Workspace) ─── */
    html = `
      <div class="clearance-timeline clearance-timeline--officer" style="display:grid;grid-template-columns:repeat(3, 1fr);gap:8px;padding:8px;background:var(--color-surface-page, var(--surface-card-alt));border-radius:8px;border:1px solid var(--color-border, var(--line));">
        <div style="text-align:center;padding:4px;">
          <div style="font-size:0.72rem;color:var(--color-text-secondary, var(--ink-secondary));font-weight:600;">Phase 1: Env</div>
          <span class="badge ${isP1Done ? "green" : "gray"}" style="font-size:0.68rem;padding:2px 6px;">${p1StatusText}</span>
        </div>
        <div style="text-align:center;padding:4px;border-left:1px solid var(--color-border, var(--line));border-right:1px solid var(--color-border, var(--line));">
          <div style="font-size:0.72rem;color:var(--color-text-secondary, var(--ink-secondary));font-weight:600;">Phase 2: Tech</div>
          <span class="badge ${isP2Done ? "green" : "blue"}" style="font-size:0.68rem;padding:2px 6px;">${p2StatusText}</span>
        </div>
        <div style="text-align:center;padding:4px;">
          <div style="font-size:0.72rem;color:var(--color-text-secondary, var(--ink-secondary));font-weight:600;">Phase 3: Apex</div>
