...
async function submitFireDecision(decision) {
  ...
  if (decision === "Approved") {
    ...
    if (checkboxes.length > 0 && checked < checkboxes.length) {
      notify(`Statutory Requirement: Please verify and tick all ${checkboxes.length} fire safety compliance checklist items before issuing provisional fire clearance.`, "warning");
      return;
    }
  }
